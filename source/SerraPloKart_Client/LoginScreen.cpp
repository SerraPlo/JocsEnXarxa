...
void LoginScreen::Build(void) {
	...
	m_loginMessage.Load("Please enter a nickname:", { 100,0,100 }, m_app->window.SDLWindow, m_app->renderer, m_app->font);
	m_loginMessage.position = { m_app->screenWidth/2- m_loginMessage.width/2, m_app->screenHeight*0.3f };

	m_loadMessage.Load("LOADING...", { 100,0,0 }, m_app->window.SDLWindow, m_app->renderer, m_app->font);
...
void LoginScreen::Update(void) {
	if (!loading) {
		...
		if (SDL_PollEvent(&evnt)) {
			...
			switch (evnt.type) {
				case SDL_TEXTINPUT:
				if (m_nickText.size() < TEXT_MAX_SIZE) m_nickText += evnt.text.text;
				break;
				case SDL_TEXTEDITING:
				break;
			}
		}
		if (m_app->inputManager.isKeyPressed(SDLK_RETURN) && !m_nickText.empty()) m_app->nick = m_nickText, loading = true;
		if (m_app->inputManager.isKeyPressed(SDLK_BACKSPACE)) m_nickText = m_nickText.substr(0, m_nickText.size() - 1);

		m_userInput.Load(m_nickText, { 0,0,100 }, m_app->window.SDLWindow, m_app->renderer, m_app->font);
		m_userInput.position = { m_app->screenWidth / 2 - (m_userInput.width / 2)*1.25f, m_app->screenHeight* 0.5f };
	} else {
		static bool assetsLoaded = false; ///TODO:temp cutre
		static float counterSend = 0;
		if (!assetsLoaded) m_app->gameObjectManager.Load(LoadAsset("gameObjects.json")), assetsLoaded = true;
		try {
			if (clock() > counterSend + MS_RESEND_DELAY) counterSend = clock(), 
...
void LoginScreen::Draw(void) {
	...
	if (!loading) {
		m_loginMessage.Draw(m_app->renderer);
		m_userInput.Draw(m_app->renderer, int(40 * m_nickText.size()));
	} else {
